...
class Test(WbxJob):
    ...
    def testddd(self):
        ...
        for dbinfo in self.source_db_info_ls:
            ...
            if db_name not in self.FEDERAMP_DB:
                ...
                # try:
                #     test_sql = '''
                #     select t.status,t.errors,to_char(actual_start_date,'yyyy-mm-dd hh24:mi:ss') actual_start_date,to_char(sysdate,'yyyy-mm-dd hh24:mi:ss') sysdate1 from dba_scheduler_job_run_details t  where job_name = 'SPLEX_MONITOR_REAL_TIME' and to_char(actual_start_date,'yyyy-mm-dd hh24:mi:ss') > '2022-04-28 12:58:00' order by t.actual_start_date desc
                #     '''
                #     logger.info("\t1.exec test_sql")
                ...
                sql = '''
                select t.status,t.errors,to_char(actual_start_date,'yyyy-mm-dd hh24:mi:ss') actual_start_date,
                ...
                and to_char(actual_start_date,'yyyy-mm-dd hh24:mi:ss') > '2022-04-28 13:20:00' order by t.actual_start_date desc
                '''
        ...
        for dbinfo in self.source_db_info_ls:
            ...
            if db_name not in self.FEDERAMP_DB:
                ...
                try:
                    ...
                    logger.info("\t2.exec schedure_sql")
                    # logger.info(monitor_adb_delay_sql)
                    cursor.execute(monitor_adb_delay_sql)
                    connect.commit()
                except Exception as e:
    ...
    def process_shareplexinfos(self,shareplexinfos):
        ...
        BEGIN
            ...
            vLogFile := UTL_FILE.FOPEN('MONITOR_LOG_DIR', vdbname||'_splex_monitor_delay.log', 'A', 32767); 
            FOR rec in (
            SELECT direction,src_db,PORT_NUMBER,to_char(logtime,'YYYY-MM-DD/hh24:mi:ss') logtime,to_char(accepttime,'YYYY-MM-DD/hh24:mi:ss') accepttime,
            ...
            LOOP
                vLogRecord := 'sp_delay,src_db_name='||rec.src_db||',tgt_db_name='||vdbname||',splex_port='||rec.port_number||',direction='||rec.direction||' delayinsecond='||rec.delayinsecond||' '||rec.sample_time;
        ...
        BEGIN
          ...
          IF v_count = 1 THEN
            NULL; 
          ELSE
             DBMS_SCHEDULER.CREATE_JOB(job_name=>'OUTPUT_SPLEX_MONITOR_REAL_TIME',job_type=>'PLSQL_BLOCK',job_action=>'BEGIN sp_output_splexmonitor_data; END;',start_date=>SYSDATE,repeat_interval=>'FREQ=MINUTELY;INTERVAL=1',enabled => TRUE,auto_drop=>FALSE);
          END IF;
        END;
        '''
