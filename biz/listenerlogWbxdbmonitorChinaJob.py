...
class ListenerlogWbxdbmonitorChinaJob(WbxJob):
    def start(self):
        config = Config()
        ...
        self.auditdao = wbxauditdbdao(self.china_depot_connectionurl)
        self.influxdbip,self.influx_database,self._client = config.getChina_InfluxDB_client()
    ...
    def main(self):
        ...
        try:
            ...
            rows = self.auditdao.getListenerlogMonitorDBInfoChina()
            print(len(rows))
        ...
        if len(data_list)>0:
            if len(data_list) > 0:
                ...
                for new_list in lists:
                    ...
                    alert_title = "Check WBXDBMONITOR_LISTENER_LOG no data in last 1 hour (%s/%s) ,China influxdb=%s" % (index, len(lists),self.influxdbip)
...
if __name__ == '__main__':
    job = ListenerlogWbxdbmonitorChinaJob()
    job.initialize()
    job.start()