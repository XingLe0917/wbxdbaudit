...
PASSWORD_EXTRA_DB=['RACAFWEB','TTA35','TTA136','RACAM1MMP','RACFRRPT','RACAMRPT']

EXCLUDE_SRC=['RACAMRPT']
EXCLUDE_TGT=['RACFRRPT','RACAMRPT']
...
class ShareplexAdbmon(WbxJob):
    ...
    def main(self):
        ...
        # all data prepared
        # Why we choose ThreadPoolExecutor instead of ProcessPoolExecutor for this task?
        # Because this task is IO-Bound task.
        # What is I/O bound task?
        # 1. An IO-bound task is a type of task that involves reading from or writing to a device, file, or socket connection.
        # 2. The operations involve input and output (IO), and the speed of these operations is bound by the device, hard drive, or network connection.
        # This is why these tasks are referred to as IO-bound.
        with ThreadPoolExecutor(max_workers=40) as exe:
            # issue some tasks and collect futures
            futures = []
            for (db, v) in src_db_tns_sql.items():
                v['db'] = db
                obj = exe.submit(self.update_src_job, v)
                futures.append(obj)
