...
class ShplexDenyMonitorJob(WbxJob):
    gserverList = []
    def start(self):
        ...
        try:
            ...
            data = []
            i = 1
            for res1 in executor.map(self.launchDenyMonitor, gserverList):
                    print("in server {}: Call launchDenyMonitor result is {}".format(i,res1))
                    i += 1

            AllProblemServerList = auditdao.getProblemShplexDenyMon()
            if len(AllProblemServerList) > 0:
                self.sendAlertMsg("All", AllProblemServerList)
                for ls in AllProblemServerList:
                    kargs = {
                        "task_type": "SPLEX_SP_OCT_DENIED_USERID",
                        "db_name": "",
    ...
    # Launch Shareplex Deny Monitor shell scripts by connect server
    def launchDenyMonitor(self,server_list):
        server = None
        ...
        try:
            ...
            if len(shplex_port)>0:
                for port in shplex_port:
                    print(port)
                    cmd2 = "sh /u00/app/admin/dbarea/bin/splex_deny_monitor_4_one_port.sh %s" %port
    ...
    def sendAlertToChatBot(self, rows_list):
        ...
        hasError = False
        msg = "### Shareplex Deny Monitor Job found problems, more info please visit PCCP\n"
        msg += "```\n {} \n```".format(rows_list)
    ...
    def sendAlertEmail(self, rows_list):
        msg = wbxemailmessage(emailtopic="Alert:Shareplex Deny Monitor Job Problems, More info:https://pccp.webex.com/#/shareplex/splex-deny-user-monitor", receiver="brzhu@cisco.com",
